...
Each OPQBox2 processes 2.7 billion samples per day. Using node level feature extraction OPQBox2's forward only 500,000 measurements to the cloud service in the same 24 hours. Further analysis and filtering yields roughly 1 distributed PQ event per day. These events contain data from every devices on the network sampled at full resolution and sampling rate.  Through the use of our multi-tiered systems we aim to bring the operators and end users, useful, intuitive, actionable, real-time information about the power grid.
